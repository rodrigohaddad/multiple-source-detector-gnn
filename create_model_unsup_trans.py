...
from gnn_embedding.gnn_unsup_trans import UTSAGE
from utils.constants import TRANSFORMED_DIR, DEVICE, MODEL_GRAPH_DIR, GRAPH_UNSUP_TRANS_FILE
...
def main():
    try:
        model = pickle.load(open(f'{MODEL_GRAPH_DIR}{GRAPH_UNSUP_TRANS_FILE}', 'rb'))
    except:
        model = UTSAGE(dim_in=8,
                       dim_h=128,
                       dim_out=128,
                       n_layers=3)
    ...
    save_to_pickle(model, 'model', 'sage_model_unsup_trans')
