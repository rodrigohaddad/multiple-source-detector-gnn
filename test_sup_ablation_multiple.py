...
from utils.constants import MODEL_GRAPH_DIR, NEW_TOP_K, MAKE_NEIGHBORS_POSITIVE, MODEL_ABLATION
...
def test(neighbors_positive=MAKE_NEIGHBORS_POSITIVE):
    ...
    for path in os.listdir('data/graph_enriched/'):
        if not path.startswith("powergrid_30inf"): #Aqui
            continue
        enriched_path = f'data/graph_enriched/{path}'
        sage_model_name = f'graph-sage-untransformed-{enriched_path.split("/")[-1]}.pickle' #Aqui
        sage = pickle.load(open(f'{MODEL_ABLATION}{sage_model_name}', 'rb')) #Aqui
    ...
    f = open(f'data/metrics_output/metrics_output_ablation_base{"_nb" if neighbors_positive else ""}.json', 'w') #Aqui
