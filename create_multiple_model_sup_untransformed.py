...
from utils.constants import DEVICE, NOT_TRANSFORMED_DIR, INFECTED_DIR
from utils.save_to_pickle import save_to_pickle


def train():
    for dirname in os.listdir(NOT_TRANSFORMED_DIR):
        if not dirname.startswith("powergrid_30inf"):
            continue
        model = SUSAGEBin(dim_in=1,  # data.num_node_features
                          dim_h=128,  # 64
                          dim_out=1,
                          n_layers=3,
                          aggr='max')
        ...
        save_to_pickle(model, 'model/untransformed', f'graph-sage-untransformed-{dirname}')
